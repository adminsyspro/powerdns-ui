...
export async function POST(request: NextRequest) {
  ...
  const { name, description, fullAccess, readOnly, zones } = body as {
    name: string;
    description?: string;
    fullAccess?: boolean;
    readOnly?: boolean;
  ...
  const insertEnv = db.prepare(
    `INSERT INTO proxy_environments (id, name, description, token_sha512, full_access, read_only)
     VALUES (?, ?, ?, ?, ?, ?)`
  );
  ...
  const transaction = db.transaction(() => {
    insertEnv.run(id, name, description || '', tokenHash, isFull ? 1 : 0, readOnly === true ? 1 : 0);
